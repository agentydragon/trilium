...
/**
 * Trilium's main entity which can represent text note, image, code note, file
 * attachment etc.
 *
...
class Note extends AbstractEntity {
  ...
  updateFromRow(row) {
    this.update([
      row.noteId, row.title, row.type, row.mime, row.isProtected,
      row.dateCreated, row.dateModified, row.utcDateCreated, row.utcDateModified
    ]);
  }

  update([
    noteId, title, type, mime, isProtected, dateCreated, dateModified,
  ...
  getChildBranches() {
    return this.children.map(
        childNote => this.becca.getBranchFromChildAndParent(
            childNote.noteId, this.noteId));
  }

  /*
   * Note content has quite special handling - it's not a separate entity, but a
   * lazily loaded part of Note entity with it's own sync. Reasons behind this
   * hybrid design has been:
   *
   * - content can be quite large and it's not necessary to load it / fill
   * memory for any note access even if we don't need a content, especially for
   * bulk operations like search
   * - changes in the note metadata or title should not trigger note content
   * sync (so we keep separate utcDateModified and entity changes records)
   * - but to the user note content and title changes are one and the same -
   * single dateModified (so all changes must go through Note and content is not
   * a separate entity)
   */
  ...
  getContent(silentNotFoundError = false) {
    const row = sql.getRow(
        `SELECT content FROM note_contents WHERE noteId = ?`, [this.noteId]);
    ...
    if (this.isProtected) {
      if (protectedSessionService.isProtectedSessionAvailable()) {
        content =
            content === null ? null : protectedSessionService.decrypt(content);
      } else {
        content = '';
      }
    }
  ...
  getScriptEnv() {
    if (this.isHtml() ||
        (this.isJavaScript() && this.mime.endsWith('env=frontend'))) {
      return 'frontend';
    }
  ...
  getAttributes(type, name) {
    ...
    if (type && name) {
      return this.__attributeCache.filter(
          attr => attr.type === type && attr.name === name);
    } else if (type) {
      ...
      if (this.noteId !== 'root') {
        for (const parentNote of this.parents) {
          parentAttributes.push(
              ...parentNote.__getInheritableAttributes(newPath));
        }
      }

      const templateAttributes = [];

      for (const ownedAttr of
               parentAttributes) {  // parentAttributes so we process also
                                    // inherited templates
  ...
  __getInheritableAttributes(path) {
    ...
    if (!this.inheritableAttributeCache) {
      this.__getAttributes(
          path);  // will refresh also this.inheritableAttributeCache
    }
  ...
  getAttributeCaseInsensitive(type, name, value) {
    ...
    return this.getAttributes().find(
        attr => attr.type === type && attr.name.toLowerCase() === name &&
            (!value || attr.value.toLowerCase() === value));
  }

  getRelationTarget(name) {
    const relation = this.getAttributes().find(
        attr => attr.type === 'relation' && attr.name === name);
   ...
   * @param {string} type - attribute type (label, relation, etc.)
   * @param {string} name - attribute name
   * @returns {Attribute} attribute of given type and name. If there's more such
   *     attributes, first is  returned. Returns null if there's no such
   *     attribute belonging to this note.
   */
   ...
   * @param {string} type - attribute type (label, relation, etc.)
   * @param {string} name - attribute name
   * @returns {string|null} attribute value of given type and name or null if no
  ...
  getOwnedAttributes(type, name) {
    ...
    if (type && name) {
      return this.ownedAttributes.filter(
          attr => attr.type === type && attr.name === name);
    } else if (type) {
  ...
  /**
   * @returns {Attribute} attribute belonging to this specific note (excludes
  ...
  hasInheritableOwnedArchivedLabel() {
    return !!this.ownedAttributes.find(
        attr => attr.type === 'label' && attr.name === 'archived' &&
            attr.isInheritable);
  }

  // will sort the parents so that non-search & non-archived are first and
  // archived at the end this is done so that non-search & non-archived paths
  // are always explored as first when looking for note path
  sortParents() {
    this.parentBranches.sort(
        (a, b) => a.branchId.startsWith('virt-') ||
                a.parentNote.hasInheritableOwnedArchivedLabel() ?
            1 :
            -1);
   ...
   *
   * @return {string} - returns flattened textual representation of note,
   *     prefixes and attributes
   */
  getFlatText() {
    if (!this.flatTextCache) {
      this.flatTextCache =
          this.noteId + ' ' + this.type + ' ' + this.mime + ' ';
      ...
      for (const attr of this.getAttributes()) {
        // it's best to use space as separator since spaces are filtered from
        // the search string by the tokenization into words
   ...
   *     include their subtrees
   *                     in effect returns notes which are influenced by note's
  ...
  getDistanceToAncestor(ancestorNoteId) {
    ...
    for (const parent of this.parents) {
      minDistance = Math.min(
          minDistance, parent.getDistanceToAncestor(ancestorNoteId) + 1);
    }
  ...
  getNoteRevisions() {
    return sql
        .getRows('SELECT * FROM note_revisions WHERE noteId = ?', [this.noteId])
        .map(row => new NoteRevision(row));
  }

  /**
   * @return {string[][]} - array of notePaths (each represented by array of
   *     noteIds constituting the particular note path)
   */
  getAllNotePaths() {
    if (this.noteId === 'root') {
      return [['root']];
   ...
   * @return {boolean} - true if ancestorNoteId occurs in at least one of the
   *     note's paths
   */
  ...
  setAttribute(type, name, value) {
    ...
    const attr =
        attributes.find(attr => attr.type === type && attr.name === name);
    ...
    if (attr) {
      ...
      new Attribute({noteId: this.noteId, type: type, name: name, value: value})
          .save();
    }
  }
  ...
  removeAttribute(type, name, value) {
    ...
    for (const attribute of attributes) {
      if (attribute.type === type && attribute.name === name &&
          (value === undefined || value === attribute.value)) {
  ...
  decrypt() {
    if (this.isProtected && !this.isDecrypted &&
      ...
      try {
        ...
        log.error(
            `Could not decrypt note ${this.noteId}: ${e.message} ${e.stack}`);
      }
    }
  }
  ...
  getPojoToSave() {
    ...
    if (pojo.isProtected) {
      if (this.isDecrypted) {
        ...
        // updating protected note outside of protected session means we will
        // keep original ciphertexts
        delete pojo.title;
      }
    }
