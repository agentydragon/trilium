...
class Note extends AbstractEntity {
    ...
    /*
     * Note content has quite special handling - it's not a separate entity, but
     * a lazily loaded part of Note entity with it's own sync. Reasons behind
     * this hybrid design has been:
     *
     ...
     * single dateModified (so all changes must go through Note and content is
    ...
    __getAttributes(path) {
        ...
        if (!this.__attributeCache) {
            ...
            for (const ownedAttr of parentAttributes) {  // parentAttributes so we process also
                // inherited templates
     ...
     * @param {string} type - attribute type (label, relation, etc.)
     * @param {string} name - attribute name
     * @returns {Attribute} attribute of given type and name. If there's more
     *     such attributes, first is  returned. Returns null if there's no such
     ...
     * @param {string} type - attribute type (label, relation, etc.)
     * @param {string} name - attribute name
     * @returns {string|null} attribute value of given type and name or null if
     ...
     * @param {string} [type] - (optional) attribute type to filter
     * @param {string} [name] - (optional) attribute name to filter
     * @returns {Attribute[]} note's "owned" attributes - excluding inherited
     *     ones
     */
    ...
    hasInheritableOwnedArchivedLabel() {
        return !!this.ownedAttributes.find(
            attr => attr.type === 'label' && attr.name === 'archived' && attr.isInheritable);
    }
    ...
    getFlatText() {
        if (!this.flatTextCache) {
            ...
            for (const attr of this.getAttributes()) {
                // it's best to use space as separator since spaces are filtered
                // from the search string by the tokenization into words
